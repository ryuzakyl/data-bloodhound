...
# TODO: This module is NO CORRECT (implement 'icoshift' from scratch)
...
# xCS, ints, ind, target = icoshift(xt='average', xp=ds.values, inter='whole', n=3)
# ds_prep = pd.DataFrame(xCS, index=ds.index, columns=ds.columns)
...
def icoshift(xt,  xp,  inter='whole',  n='f', scale=None, coshift_preprocessing=False,
    ...
    if scale is None:
        ...
        scale = numpy.array(list(range(0, xp.shape[1])))
    ...
    if using_custom_scale:
        ...
        if flag_di_scale:
            logging.warning('Scale vector is not continuous, the defined intervals might not reflect actual ranges')
    ...
    if isinstance(inter, str):
        ...
        if inter == 'whole':
            ...
            raise NotImplementedError('Left empty for future correct implementation.')
            # interv = regexp(inter, '(-{0,1}\\d*\\.{0,1}\\d*)-(-{0,1}\\d*\\.{0,1}\\d*)', 'tokens')
            # interv = sorted(scal2pts(float(cat(0, interv[:])), scale, prec))
            #
            # if interv.size != 2:
            #     raise Exception
            #
            # inter = list(range(interv[0], (interv[1] + 1)))
            # flag2 = True
        ...
        flag2 = numpy.array_equal(numpy.fix(inter), inter) and \
                max(inter.shape) > 1 and \
                numpy.array_equal(numpy.array([1, numpy.max(inter) - numpy.min(inter) + 1]).reshape(1, -1), inter.shape) and \
                numpy.array_equal(numpy.unique(numpy.diff(inter, 1, 2)), 1) # CHANGE: added numpy.
    ...
    if isinstance(inter, str) and n not in ['b', 'f']:
        ...
        if scfl and not isinstance(n, int):
            logging.warning('"n" must be an integer if scale is ignored; first element (i.e. %d) used' % n)
            n = numpy.round(n)
        else:
            if using_custom_scale:
    ...
    if vec(flag).any():
        ...
        if flag2:
            ...
            if intern_.shape[0] != 1:
                ...
                logging.warning('The missing values at the boundaries of the reference region will be ignored')

            intern_ = list(range(inter[0] + intern_[0], (inter[0] + intern_[1] + 1)))
        else:
        ...
        if max(intern_.shape) != max(inter.shape) and not flag2:
            if whole:
                ...
                logging.warning('To handle the pattern of missing values, %d segments are created/removed' % (abs(max(intern_.shape) - max(inter.shape)) / 2))
                inter = intern_
                nint, mint = inter.shape
    xcs = xp
    mi, pmi = min_with_indices(inter)
    ...
    flag = max(inter.shape) > 1 and numpy.array_equal(
        # CHANGE: added numpy.
        numpy.array([1, pma - pmi + 1]).reshape(1, -1), inter.shape) and numpy.array_equal(numpy.unique(numpy.diff(inter, 1, 2)), 1)
    ...
    if flag:
        ...
        ind = nans(np, 1)
        # CHANGE: removed the second parameter from all built-in function
        # missind = not all(numpy.isnan(xp), 2)
        missind = not all(numpy.isnan(xp))
        ...
        if numpy.any(intdif[1:2:max(intdif.shape)] < 0):
            logging.warning('The user-defined intervals are overlapping: is that intentional?')
    ...
    if frag:
        ...
        for i_sam in range(0, np):
            for i_seg in range(0, in_or.shape[0]):
                xn[i_sam, in_or[i_seg, 0]:in_or[i_seg, 1] + 1] = xcs[i_sam, in_or[i_seg, 2]:in_or[i_seg, 3] + 1]
                if loc_ind[i_sam] < 0:
                    if flag_nan[i_seg, 0, i_sam]:
                        xn[i_sam, in_or[i_seg, 0]:in_or[i_seg, 0] - loc_ind[i_sam, 0] + 1] = numpy.nan
                else:
                    if loc_ind[i_sam] > 0:
    ...
    if auto == 1:
        while auto == 1:
            if filling == -numpy.inf:
                ...
                # FIXME
                xtemp = cat(1, nans(np, n), xp, nans(np, n))
        ...
        for i_block in range(n_blocks):
            ...
            block_indices = list(range(ind_blocks[i_block], ind_blocks[i_block + 1]))
        ...
        if numpy.max(abs(ind)) == n:
            logging.warning('Scrolling window size "n" may not be enough wide because extreme limit has been reached')
...
def defints(xp, interv):
    ...
    logging.warning('The last interval will not fulfill the selected intervals size "inter" = %f' % interv)

    if plus >= 0:
        logging.warning('Size of the last interval = %d ' % plus)
    else:
        logging.warning('Size of the last interval = %d' % (interv + plus))
    ...
    t = cat(1, list(range(0, (mp + 1), interv)), mp)
    if t[-2] == t[-2]:
        t[-2] = numpy.array([])
...
def cc_fft_shift(t, x=False, options=numpy.array([])):
    ...
    if (options[0] < 0) and (options[1] > 0):
        ...
        ind = list(range(int(len_fft + options[0]), int(len_fft + options[1] + 1))) + [1]

    else:
        # ind = Options(1) + 1:Options(2) + 1
        ind = list(range(int(options[0]), int(options[1] + 1)))
    ...
    for i_X in range(0, dim_x[0]):
        ...
        if shift[i_X] >= 0:
            ...
            if options[4] == - numpy.inf:
                ...
                if len(o) > 0:
                    ind = cat(1, ind, numpy.array(dim_x[time_dim[o]] - 1).reshape(1, -1))
                    indw = cat(1, indw, numpy.arange(dim_x[time_dim] - shift[i_X], dim_x[time_dim]).reshape(1, -1))
...
def scal2pts(ppmi, ppm=[], prec=None):
    ...
    # CHANGE: Commented this line
    # rev = ppm[0] > ppm[1]

    if prec is None:
        # CHANGE: added the numpy.
        prec = min(abs(numpy.unique(numpy.diff(ppm))))

    pts = []
    for i in ppmi:
        # CHANGE: added the numpy.array() surrounding ppm
        nearest_v, idx = find_nearest(numpy.array(ppm), i)
